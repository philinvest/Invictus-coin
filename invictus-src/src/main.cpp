...
uint256 hashGenesisBlock("0x67e3d3614287f7a7edb3696bab881554063f9e75d01ac0bddfc60da1e71c9808");
...
{
    // invictus: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // invictus: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
	    ...
	    assert(block.hashMerkleRoot == uint256("0x9ec48a98eca7ff371ac7f5a4411d1d3077b700884333d4ee8a2bc07f97a7347c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xde, 0xca, 0xa4, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("invictusMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static invictusMiner(CWallet *pwallet)
{
    printf("invictusMiner started\n");
    ...
    try { loop {
        ...
        printf("Running invictusMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("invictusMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&invictusMiner, pwallet));
}
